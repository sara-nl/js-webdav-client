...
(function() { //This creates a scope where I can put private functions (NOT private properties, as these will be static!)
  ...
  nl.sara.webdav.Client.prototype.get = function(path, callback) {
    ...
    // And then send the request
    var ajax = null;
    ajax = getAjax("GET", url, function(status) {
  ...
  nl.sara.webdav.Client.prototype.head = function(path, callback) {
    ...
    // And then send the request
    var ajax = null;
    ajax = getAjax("HEAD", url, function(status) {
  ...
  nl.sara.webdav.Client.prototype.post = function(path, callback, body, contenttype) {
    ...
    // And then send the request
    var ajax = null;
    ajax = getAjax("POST", url, callback);
    if (body !== undefined){
      if (contenttype !== undefined) {
        ajax.setRequestHeader('Content-Type', contenttype);
      }else{
   ...
   * @param   boolean                       noOverwrite  If set to true, overwrites are not allowed. Else potential destination resources are overwritten without warning
   ...
   * @return  Client                                     The client itself for chaining methods
   */
  nl.sara.webdav.Client.prototype.copy = function(path, callback, destination, noOverwrite, depth) {
    ...
    // And then send the request
    var ajax = getAjax("COPY", url, callback);
   ...
   * @return  Client                                     The client itself for chaining methods
   */
  nl.sara.webdav.Client.prototype.move = function(path, callback) {
    throw new nl.sara.webdav.Exception('Not implemented yet', nl.sara.webdav.Exception.NOT_IMPLEMENTED);
    if ((path === undefined) || (callback === undefined)) {
      throw new nl.sara.webdav.Exception('MOVE requires the parameters path and callback', nl.sara.webdav.Exception.MISSING_REQUIRED_PARAMETER);
    }
    if (typeof path != "string"){
      throw new nl.sara.webdav.Exception('MOVE parameter; path should be strings', nl.sara.webdav.Exception.WRONG_TYPE);
    }
    ...
    // And then send the request
    var ajax = getAjax("MOVE", url, callback);
    ajax.send();
